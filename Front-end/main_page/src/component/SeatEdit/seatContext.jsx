...
export default function SeatProvider({ children, apiLoc }) {
  ...
  useEffect(() => {
    const fetchData = async () => {
      if (musical_id == undefined) {
        try {
          const response = await fetch(`https://muu-t.onrender.com/api/seat/getseat/${hall_id}`);
          if (!response.ok) {
      ...
      if (hall_id == undefined) {
        try {
          const response = await fetch(
            `https://muu-t.onrender.com/api/seat/getseatposition/${musical_id}`
          );
          const response2 = await fetch(
            `https://muu-t.onrender.com/api/seat/getseat/musical/${musical_id}`
          );
          if(!response.ok||!response2.ok){
            const response_hall = await fetch(
              `https://muu-t.onrender.com/api/seat/gethall/${musical_id}`
            ) 
  ...
  const save_seats = async () => {
    ...
    let uri = hall_id
      ? `https://muu-t.onrender.com/api/seat/saveposition/h/${hall_id}`
      : `https://muu-t.onrender.com/api/seat/saveposition/m/${musical_id}`;
    try {
